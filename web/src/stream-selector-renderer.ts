...
export class StreamSelectorRenderer {
  ...
  private async createPipelines() {
    ...
    // Compute Shader: Calculates positions
    const computeShader = `
      struct StreamInfo {
        atlasIndex: f32,
      }
    ...
    const renderShader = `
      ...
      fn vs_main(@builtin(vertex_index) vertexIndex: u32, @builtin(instance_index) instanceIndex: u32) -> VertexOutput {
        ...
        // Culling (move offscreen if not visible)
        if (pos.visible < 0.5) {
          return VertexOutput(vec4f(-2.0, -2.0, 0.0, 1.0), vec2f(0.0), 0.0);
        }
        ...
        // Scale and translate
        let x = -1.0 + vPos.x * 2.0; // Full width
        // Map Y from pixels to Clip Space (-1 to 1)
        // Y starts at top (1.0) and goes down
      ...
      fn fs_main(in: VertexOutput) -> @location(0) vec4f {
        // Sample text atlas
        // Atlas has many rows. we need to find the correct row based on atlasIndex.
        // UV.y is 0..1 for this item.
        // We map it to the atlas UV.

        let atlasHeight = 4096.0; // Hardcoded from TextAtlas
        let itemHeight = 40.0;
        ...
        // Background color
        let bgColor = vec4f(0.1, 0.1, 0.15, 1.0);
        let hoverColor = vec4f(0.2, 0.2, 0.3, 1.0); // Simulating hover is hard without mouse pos uniform

        // Simple alpha blending for text
        return mix(bgColor, vec4f(1.0), textCol.a);
      }
    `
  ...
  setStreams(streams: Stream[]) {
    ...
    for (const s of streams) {
      this.textAtlas.addText(`${s.name} ${s.quality || ''}`)
    }
    this.textAtlas.upload(this.device)

    // 2. Create/Update Buffers
    // Stream Info Buffer (atlas index)
    const streamData = new Float32Array(streams.map(s => this.textAtlas.addText(`${s.name} ${s.quality || ''}`)))
    this.streamBuffer = this.device.createBuffer({
      size: Math.max(streamData.byteLength, 16), // Min size
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    })
    this.device.queue.writeBuffer(this.streamBuffer, 0, streamData)

    // Position Buffer (computed)
    // 2 floats per stream (y, visible) -> 8 bytes
    const posSize = Math.max(streams.length * 8, 16)
  ...
  private createBindGroups() {
    ...
    if (!this.streamBuffer || !this.positionBuffer || !this.uniformBuffer || !this.textAtlas.texture) return
  ...
  private update(dt: number) {
    ...
    const factor = 1.0 - Math.pow(0.1, dt / 16.0) // approx 0.1 per 16ms frame
  ...
  private render() {
    ...
    // Draw 4 vertices per instance, instances = stream count
  ...
  // Method to get stream at position (for click handling)
  getStreamAt(y: number): Stream | null {
    // Inverse of y = index * itemHeight - scrollY
    // y + scrollY = index * itemHeight
    // index = (y + scrollY) / itemHeight
