...
const ENV_VAR =
  'cross-env STREAMS_DIR=tests/__data__/input/playlist_generate DATA_DIR=tests/__data__/input/data PUBLIC_DIR=tests/__data__/output/.gh-pages LOGS_DIR=tests/__data__/output/logs ROOT_DIR=tests/__data__/output'
...
describe('playlist:generate', () => {
  it('can generate playlists and logs', () => {
    ...
    const playlists = glob
      .sync('tests/__data__/expected/playlist_generate/**/*.m3u')
      .map(filepath => {
...
function content(filepath: string) {
  return fs.readFileSync(pathToFileURL(filepath), { encoding: 'utf8' }).replace(/\r\n/g, '\n')
}
